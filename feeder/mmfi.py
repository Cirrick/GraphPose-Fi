...
class MMFi_Dataset(Dataset):
    ...
    def load_test_iid_data(self):
        ...
        seg_csv_path = '/root/autodl-tmp/Dataset/MMFi/MMFi_Dataset/MMFi_action_segments.csv'
    ...
    def load_test_data(self):
        data_info = []
        # read mmfi_action_segments.csv
        seg_csv_path = os.path.expanduser('~/Human-sensing/Dataset/MMFi/MMFi_Dataset/MMFi_action_segments.csv')
        seg_csv = pd.read_csv(seg_csv_path)
        # 初始化字典
        action_dict = defaultdict(lambda: {'start': [], 'end': []})
        # 解析并填充字典
