...
def calulate_error(preds, gts, align=False):
    ...
    mpjpe_joints = np.sqrt(np.sum(np.square(preds - gts), axis=2))  #  b 17
    mpjpe = mpjpe_joints.mean(axis=1)

    pampjpe_joints = np.zeros([N, num_joints])
    ...
    for n in range(N):
        ...
        pampjpe_joints[n] = np.sqrt(np.sum(np.square(frame_pred - frame_gt), axis=1))

    # pampjpe = np.mean(pampjpe)
    pampjpe = pampjpe_joints.mean(axis=1)

    mpjpe_per_joint   = mpjpe_joints.mean(axis=0) 
    pampjpe_per_joint = pampjpe_joints.mean(axis=0) 

    return mpjpe, pampjpe, mpjpe_per_joint, pampjpe_per_joint

def log_config(config: dict, output_dir: str, writer=None):
    if writer is not None:
        text = "\n".join([f"{k}: {v}" for k, v in config.items()])
        writer.add_text("Config/Hyperparams", text, global_step=0)

def param_groups_simple(model, lr=3e-4, wd=0.02, extra_no_decay_keywords=()):
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if (name.endswith(".bias") or p.ndim == 1 or any(k in name for k in extra_no_decay_keywords)):
            no_decay.add(p)
        elif p not in no_decay:
            decay.add(p)

    return [
        {'params': list(decay),    'lr': lr, 'weight_decay': wd},
        {'params': list(no_decay), 'lr': lr, 'weight_decay': 0.0},
    ]

edges_17 = torch.tensor([
    [0, 1], [1, 2], [2, 3],
    [0, 4], [4, 5], [5, 6],
    [0, 7], [7, 8], [8, 9], [9,10],
    [8,11], [11,12], [12,13],
